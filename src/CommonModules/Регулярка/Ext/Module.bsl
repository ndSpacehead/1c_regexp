		...
		Для Каждого Вершина Из ЭлементыМешка(Вершины) Цикл
			Если Вершина < Финал Тогда
				...
				Если ТипТокена = ТипСтруктура Тогда
					Если Токен.Тип = РегуляркаПовтИсп.ТипТокенаКлассСимволов() И СимволУдовлетворяетКлассу(Символ, Токен) Тогда
...
Функция ТокеныШаблона(Знач Шаблон)
	...
	Для я = 1 По СтрДлина(Шаблон) Цикл
		...
		ИначеЕсли Символ = "." Тогда
			фРезультат.Добавить(НовыйТокенОбъект(РегуляркаПовтИсп.ТипТокенаКлассСимволов(), НовыеДанныеТокенаКлассаЛюбыхСимволов()));
		Иначе
...
Процедура ОбработатьКлассСимволов(Токены, Индекс, Знач Шаблон)
	...
	Токен = НовыйТокенОбъект(РегуляркаПовтИсп.ТипТокенаКлассСимволов(), НовыеДанныеТокенаКлассаСимволов(Отрицание));
...
Процедура ОбработатьЭкранированныйСимволКласса(СимволыКласса, Индекс)
	...
	ИначеЕсли СимволыКласса[Индекс] = "u" Тогда // 0xhhhh
		...
		Индекс = Индекс + 4;
	Иначе
		Символ = РегуляркаПовтИсп.ЭкранированияСимволовВКлассе().Получить(СимволыКласса[Индекс]);
...
Процедура ОбработатьЭкранирование(Токены, Индекс, Знач Шаблон)
	...
	Символ = РегуляркаПовтИсп.ЭкранированияСимволов().Получить(СледующийСимвол);
	Если Символ <> Неопределено Тогда
		Токены.Добавить(Символ);
		Возврат;
	КонецЕсли;
	
	ТокеныКласса = РегуляркаПовтИсп.ПредопределенныеКлассыСимволов().Получить(СледующийСимвол);
...
Функция ШвД(Знач ЧислоHex)
	...
	ЗначенияDec = РегуляркаПовтИсп.ЗначенияЦифрHexВDec();
